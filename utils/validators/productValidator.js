...
exports.createProductValidator = [
    ...
    check("sold")
        .optional().isNumeric().withMessage("Product quantity must be a number"),
    check("price")
    ...
    check("subCategories")
        ...
        .custom((subsArr, { req }) =>
            SubCategory.find({ category: req.body.category }).then(
                (subsBelongToCat) => {
                    // Get all subcategories related to this category in db
                    const subsIdsInDB = [];
                    subsBelongToCat.forEach((subCategory) => {
                        subsIdsInDB.push(subCategory._id.toString());
                    });
                    // Check if all subcategories in the array are related to this category
                    const checker = (target, arr) => target.every((v) => arr.includes(v));
                    if (!checker(subsArr, subsIdsInDB)) {
                        return Promise.reject(
